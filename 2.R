...
server <- function(input, output, session) {
    ...
    output$piper_plot <- renderPlot({
      ...
      chem_data <- data_storage$csv_data %>%
        filter(
      ...
      piperDF <- data.frame(
        ...
        Mg = chem_data$MG,
        Na = chem_data$Sodium,
        Cl = chem_data$CHLORIDE,
        SO4 = chem_data$SULPHATE,
        HCO3 = chem_data$BICARBONATE
      )
      piperPlot(piperDF)
    })
    
    # Chemistry Composition Plot
    output$chemCompPlot <- renderPlot({
      ...
      chem_data <- data_storage$csv_data %>%
        ...
        summarise(
          Calcium = mean(CA, na.rm = TRUE),
          Magnesium = mean(MG, na.rm = TRUE),
          Sodium = mean(Sodium, na.rm = TRUE),
          Chloride = mean(CHLORIDE, na.rm = TRUE),
          Sulphate = mean(SULPHATE, na.rm = TRUE),
          Bicarbonate = mean(BICARBONATE, na.rm = TRUE)
        ) %>%
        pivot_longer(everything(), names_to = "Ion", values_to = "Value")
      
      ggplot(chem_data, aes(x = Ion, y = Value, fill = Ion)) +
        geom_bar(stat = "identity") +
        theme_minimal() +
        labs(title = "Average Chemical Composition", y = "Concentration (mg/L)", x = NULL)
    })
  }
