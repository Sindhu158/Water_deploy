...
# Integrated Piper Plot with Shiny Dashboard (within "Ground Water Chemistry" tab)

# Required Libraries (already loaded in your existing script):
  ...
  output$piper_plot <- renderPlot({
    ...
    tryCatch({
      ...
      piperPlot(df$Ca.meq, df$Mg.meq, df$Na.meq,
                ...
                Plot = list(name = df$SS, color = setColor(df$SS)),
                zCat.title = "Sodium",
                xAn.title = "Chloride",
                yAn.title = "Bicarbonate")
      
    }, error = function(e) {
      showNotification("Error generating Piper Plot: Check data format.", type = "error")
      return(NULL)
    })
  })
